...
function changeState(newState)
	if states[newState] then
		...
		typeText(text or "ATENÇÃO! TEXTO EM FALTA PARA O ESTADO " .. tostring(newState))
	else
		typeText(textData, "ERRO! " .. tostring(newState) .. " ESTADO NÃO DEFINIDO!")
	end
end
...
function buttonPress(button)
	if button then
		...
		elseif buttonType == "Upgradable" then
			...
			typeText("ERRO! BOTÃO INVÁLIDO PRESSIONADO")
		end
	end
end
